data{
  ...
  // indices
  int fdiv[ND];  // diversity of the plot where the focal tree is located (need to be centered)
...
parameters{
  ...
  real gamma_intra; // one for intra-specific interaction
...
model{
  ...
  for(i in 1:ND){    // loop over all data points
        ...
        for(j in 1:NN){    // sum over all neighbors
		  ...
		  if (fsp_ID[i] != neib_sp_ID[i, j])
          interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]+gamma[fsp_ID[i], neib_sp_ID[i,j]]*log2(fdiv[i]/div_ave)) * neib_bm[i,j]^b;			  
        }
        // prediction
