...
model{
  ...
  for(i in 1:ND){    // loop over all data points
        ...
        for(j in 1:NN){    // sum over all neighbors
			...
			interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]*(fdiv[i]/div_ave)^gamma_intra) * neib_bm[i,j]^b;	
		    
			if (alpha[fsp_ID[i], neib_sp_ID[i,j]]>0)
            interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]*(fdiv[i]/div_ave)^gamma_pos) * neib_bm[i,j]^b;	
			
			if (alpha[fsp_ID[i], neib_sp_ID[i,j]]<0)
            interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]*(fdiv[i]/div_ave)^gamma_neg) * neib_bm[i,j]^b;			
        }
        // prediction
