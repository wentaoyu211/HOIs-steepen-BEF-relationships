...
model{
  ...
  real gamma;         // for conditional operator
  ...
  for(i in 1:ND){    // loop over all data points
		...
		gamma = alpha[fsp_ID[i], neib_sp_ID[i, j]]>0 ? gamma_pos : gamma_neg;
          interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]+gamma*log2(fdiv[i]/div_ave)) * neib_bm[i,j]^b;	
        }
        // prediction
