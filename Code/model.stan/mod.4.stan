...
parameters{
  real gamma_ave; // one slope for all insteraction ~ div relationship
...
model{
  ...
  for(i in 1:ND){    // loop over all data points
        ...
        for(j in 1:NN){    // sum over all neighbors
          interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]+gamma_ave*log2(fdiv[i]/div_ave)) * neib_bm[i,j]^b;	
        }
        // prediction
