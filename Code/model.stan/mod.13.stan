...
parameters{
  real gamma_intra; // one slope for intra ~ div relationship 
  real gamma_inter; // one slope for inter ~ div relationship 
...
model{
  ...
  for(i in 1:ND){    // loop over all data points
        ...
        for(j in 1:NN){    // sum over all neighbors
			...
			if (fsp_ID[i] != neib_sp_ID[i, j])
            interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]+ gamma_inter * log2(fdiv[i]/div_ave)) * neib_bm[i,j]^b;	
        }
        // prediction
