data{
  ...
  real div_ave; // average diversity level for estimated alphas
  ...
  // indices
  int fdiv[ND];  // diversity of the plot where the focal tree is located (need to be centred)
...
parameters{
  real gamma_pos; //  slope for positive insteraction strengths
  real gamma_neg; // slope for negative interaction strengths
  // random effects
...
model{
  ...
  // specify priors
  gamma_pos ~ normal(0, 1);
  gamma_neg ~ normal(0, 1);
  ...
  for(i in 1:ND){    // loop over all data points
        ...
        for(j in 1:NN){    // sum over all neighbors
		  ...
		  if ( alpha[fsp_ID[i], neib_sp_ID[i,j]] > 0)
          interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]+gamma_pos*(fdiv[i]-div_ave)) * neib_bm[i,j]^b;	
		  
		  else if ( alpha[fsp_ID[i], neib_sp_ID[i,j]] < 0)
          interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]+gamma_neg*(fdiv[i]-div_ave)) * neib_bm[i,j]^b;	
