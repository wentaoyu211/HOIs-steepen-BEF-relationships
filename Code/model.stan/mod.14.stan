data{
  ...
  // indices
  int fdiv[ND];  // need to take log2(div) then centered around log2(4) 
...
model{
  ...
  for(i in 1:ND){    // loop over all data points
        ...
        for(j in 1:NN){    // sum over all neighbors
		    if (fsp_ID[i] == neib_sp_ID[i, j])
			interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]+gamma_intra*log2(fdiv[i]/div_ave)) * neib_bm[i,j]^b;	
		    
			if (alpha[fsp_ID[i], neib_sp_ID[i,j]]>0)
            interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]+gamma_pos*log2(fdiv[i]/div_ave)) * neib_bm[i,j]^b;	
			
			if (alpha[fsp_ID[i], neib_sp_ID[i,j]]<0)
            interactions = interactions + (alpha[fsp_ID[i], neib_sp_ID[i,j]]+gamma_neg*log2(fdiv[i]/div_ave)) * neib_bm[i,j]^b;			
        }
        // prediction
